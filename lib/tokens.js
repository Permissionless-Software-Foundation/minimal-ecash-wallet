...
class Tokens {
  ...
  async listTokensFromAddress (addr) {
    try {
      ...
      // Convert to a eCash address.
      if (addr.includes('simpleledger')) {
        addr = this.bchjs.SLP.Address.toCashAddress(addr)

        addr = this.bchjs.Address.toEcashAddress(addr)
      // } else if(addr.includes('ecash') || addr.includes('etoken')) {
      //   addr = this.bchjs.Address.ecashtoCashAddress(output.address)
      // }
      } else if (addr.includes('bitcoincash') || addr.includes('etoken')) {
        addr = this.bchjs.Address.toEcashAddress(addr)
      }
      // console.log(`listTokensFromAddress() final addr: ${addr}`)
  ...
  async createTransaction (
    ...
    try {
      ...
      if (output.address.includes('ecash') || output.address.includes('etoken')) {
        output.address = this.bchjs.Address.ecashtoCashAddress(output.address)
      }
      ...
      if (slpSendObj.outputs > 1) {
        transactionBuilder.addOutput(
          this.bchjs.SLP.Address.toLegacyAddress(walletInfo.cashAddress),
          546
        )
      }
  ...
  async createBurnTransaction (
    ...
    try {
      ...
      // Convert different addresses to legacy address
      // let legacyAddr = ''
      // if()
